...
#### [Exercise]: Build a BioMart query and annotate the DE results
...
<details>
    ...
    First, create a new table called `res_WT_anno` that includes a column with the ENSEMBL ids named `genes` using the `mutate` function. Then use the `join` function to combine the `GeneKey` table with the `res_WT` DE results. 
```{r updateDETableTidyr}
## TODO: CHECK TIDYVERSE SOLUTION

res_WT_anno <- res_WT %>%
  mutate(genes = row.names(res_WT_anno))
...
res_WT_anno <- merge(GeneKey, res_WT_anno, by.x = "external_gene_id", by.y="genes", all.x = FALSE, all.y = TRUE)
...
If we generated another comparsion, we could repeat our annotations or write the DE results directly to file.
